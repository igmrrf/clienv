...
import { areAttributesDifferent } from "./helpers";
import {
	logError,
	logInfo,
	logWarn,
...
const checkEnvFile = async (config = DEFAULT_OPTIONS) => {
	...
	try {
		...
		logInfo(
			`✅ Schema file (${chalk.underline(getShortFileName(schemaFile))}) checked successfully`,
		);
	} catch (err) {
	...
	if (isDifferentContent) {
		logWarn(
			` !  Environment file (${chalk.underline(getShortFileName(envFile))}) differs from ${chalk.underline(getShortFileName(schemaFile))}`,
		);
		await updateEnvFile(
			options,
			schemaContent,
	...
	logInfo(
		`✅ Environment file (${chalk.underline(getShortFileName(envFile))}) checked successfully`,
	);
	exitWithSuccess();
};
