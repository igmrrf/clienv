...
const { checkEnvFile } = require("../../../../dotenv-checker/src");
const { version } = require("../../../../dotenv-checker/package.json");

program
	.name("dotenv-checker")
	.description(
		"🔖 CLI to check .env files depending on a schema file with some builtin functionalities",
	)
	...
	.addOption(
		new Option("-e, --env <env-file>", "path/name of the .env file").default(
			".env.local",
			".env.local",
		),
	)
...
const envFilePath = path.isAbsolute(env) ? env : path.join(workingDir, env);
...
const config = {
	...
	options: {
		skipCreateQuestion: !!(
			skipCreateQuestion === true || skipCreateQuestion === "true"
		),
		skipUpdateQuestion: !!(
			skipUpdateQuestion === true || skipUpdateQuestion === "true"
		),
	},
};
